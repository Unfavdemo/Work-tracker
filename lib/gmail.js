...
async function isStudentEmail(message, accessToken) {
  try {
    ...
    const educationalDomains = ['.edu', 'student', 'university', 'college', 'school']
...
export async function getStudentEmailStats(accessToken, options = {}) {
  try {
    ...
    const studentQuery = `${dateQuery} (${keywordQuery})`
    ...
    const weeklyStudentQuery = `${weeklyDateQuery} (${keywordQuery})`
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayStudentQuery = `${dayDateQuery} (${keywordQuery})`
    ...
    const previousStudentQuery = `${previousDateQuery} (${keywordQuery})`
